# Done by BHADAURIA, Aditya Singh. SID: 21096478
"""
...
def add_lava():
    ...
    if game_being_played:
        for i in range(quantity_of_lava_cells_to_add_each_time):
            ...
            if lava_front_row == player_row and lava_front_col == player_col: # Check if the newly added lava has touched the player
...
def check_possible_movement(row, col):
    ...
    if col<0 or col>total_cols-1 or row<0 or row>total_rows-1 or map[row][col] == 1:
        return False
    else:
        return True
...
def handle_if_gold(row, col):
    ...
    if game_being_played:
        ...
        if map[row][col] == 2:
            ...
            print("The bird has collected",str(player_gold_coins),"out of",str(target_gold_coins),"gold coins!")
